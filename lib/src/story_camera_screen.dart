...
class _StoryCameraScreenState extends State<StoryCameraScreen>
  ...
  Future<void> _setupCameraController(CameraDescription camera) async {
    ...
    // Check if audio permission is granted, otherwise disable audio to prevent init crashes
    bool audioEnabled = false;
    try {
      final micStatus = await Permission.microphone.status;
      audioEnabled = micStatus.isGranted;
    } catch (_) {
      audioEnabled = false;
    }
    ...
    // Attempt 1: High resolution with audio if permitted
    try {
      ...
      debugPrint('Camera init attempt 1 (high, audio=$audioEnabled) failed: $e');
      await controller?.dispose();
      controller = null;
    }

    // Attempt 2: Fallback to High resolution without audio
    if (!initializedSuccessfully && audioEnabled) {
      try {
        controller = createController(
          resolution: ResolutionPreset.high,
          enableAudio: false,
        ...
        debugPrint('Camera init attempt 2 (high, no-audio) failed: $e');
        await controller?.dispose();
        controller = null;
      }
    }

    // Attempt 3: Fallback to Medium resolution without audio
    if (!initializedSuccessfully) {
      try {
        ...
        debugPrint('Camera init attempt 3 (medium, no-audio) failed: $e');
        await controller?.dispose();
        controller = null;
      }
    }

    if (!initializedSuccessfully || controller == null) {
    ...
    // Safely apply optional device configurations (never abort initialization on failure)
    try {
      await _cameraController!.lockCaptureOrientation(
        DeviceOrientation.portraitUp,
      );
    } catch (e) {
      debugPrint('lockCaptureOrientation not supported: $e');
    }
    ...
    try {
      ...
      debugPrint('Zoom level query failed: $e');
      _minZoom = 1.0;
      _maxZoom = 1.0;
      _zoomLevel = 1.0;
    }
    ...
    debugPrint('Camera initialized successfully: ${_cameraController!.value.previewSize}');
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              config.strings.cameraCouldNotTakePhoto,
            ...
            ElevatedButton(
              ...
              child: Text(config.strings.cameraGrantPermission),
            ),
          ],
        ),
      );
